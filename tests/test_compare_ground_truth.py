"""
This model does not contain proper unit tests.
It compares waveforms generated by this package (dingo-waveform) with waveforms generated by the original
Dingo packages. To be more precise: waveforms and intermediate data structures.
Waveform generation was run using the Dingo packages,
and some data structure dumped into pickle files.
...
def test_inspiral_choose_fd_modes_parameters() -> None:
    # this compares intermediate values,
    # i.e. the arguments passed to SimInspiralChooseFDModes,
    # as well as the result of the processing of its output
    # (linked_list_modes_to_dict_modes, convert_J_to_L0_frame)

    # configuration for the waveform generation
    p = {
    ...
    # the arguments to be passed to SimInspiralChooseFDModes
    ...
    # checking the arguments we just generated are the same
    # as the one obtained when using 'original' dingo.
    assert len(args_ground_truth) == len(args)
    for a1, a2 in zip(args, args_ground_truth):
        assert type(a1) == type(a2)
        assert a1 == a2

    # reading the ground truth pickle file
    ...
    # computing the corresponding value using the 'current' package
    hlm_fd: SphHarmFrequencySeries = SimInspiralChooseFDModes(*args)
    hlm_fd_: Dict[Modes, COMPLEX16FrequencySeries] = (
    ...
    hlm_fd___: Dict[Modes, FrequencySeries] = (
        inspiral_choose_fd_modes_parameters.convert_J_to_L0_frame(hlm_fd__)
    )

    # comparing 'original' dingo and 'current' implementation.
