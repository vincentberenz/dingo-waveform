"""Tests for waveform transform pipeline."""
...
class TestApplySVD:
    """Test cases for ApplySVD transform."""
    ...
    def sample_polarizations(self):
        """Create sample polarization dictionary."""
        np.random.seed(42)
        return {
    ...
    def test_compress_transform(self, sample_basis, sample_polarizations):
        """Test SVD compression transform."""
    ...
    def test_decompress_transform(self, sample_basis):
        """Test SVD decompression transform."""
        # Create compressed data
        compressed = {
            "h_plus": np.random.randn(10, 20) + 1j * np.random.randn(10, 20),
            "h_cross": np.random.randn(10, 20) + 1j * np.random.randn(10, 20),
        }
    ...
    def test_roundtrip(self, sample_basis, sample_polarizations):
        ...
        assert reconstructed["h_cross"].shape == sample_polarizations["h_cross"].shape

        # Check reconstruction works (shape and type preserved)
        # Note: Reconstruction quality depends on how well the data matches
        # the training data used for the SVD basis
...
class TestWhitenAndUnwhiten:
    ...
    def asd_file(self, tmp_path):
        """Create a temporary ASD file."""
        domain = UniformFrequencyDomain(f_min=20.0, f_max=1024.0, delta_f=0.25)
        asd = np.ones(len(domain)) * 1e-23  # Constant ASD for simplicity
    ...
    def sample_polarizations_whitening(self, asd_file):
        """Create sample polarizations matching ASD domain."""
        domain, _ = asd_file
        np.random.seed(42)
        return {
            "h_plus": np.random.randn(5, len(domain)) + 1j * np.random.randn(5, len(domain)),
            "h_cross": np.random.randn(5, len(domain)) + 1j * np.random.randn(5, len(domain)),
        }
    ...
    def test_whiten_transform(self, asd_file, sample_polarizations_whitening):
        ...
        assert "h_plus" in result
        assert "h_cross" in result
        assert result["h_plus"].shape == sample_polarizations_whitening["h_plus"].shape
    ...
    def test_whitening_roundtrip(self, asd_file, sample_polarizations_whitening):
        ...
        assert np.allclose(
            reconstructed["h_plus"], sample_polarizations_whitening["h_plus"]
        )
        assert np.allclose(
            reconstructed["h_cross"], sample_polarizations_whitening["h_cross"]
        )

    def test_error_nonexistent_file(self):
        """Test error with non-existent ASD file."""
        domain = UniformFrequencyDomain(f_min=20.0, f_max=1024.0, delta_f=0.25)
...
class TestComposeTransforms:
    ...
    def test_realistic_pipeline(self, tmp_path):
        """Test realistic pipeline with SVD and whitening."""
        # Create domain and ASD
        domain = UniformFrequencyDomain(f_min=20.0, f_max=512.0, delta_f=0.5)
        ...
        # Should be whitened and compressed
