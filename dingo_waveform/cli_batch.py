  ...
  dingo-verify-batch --seed 42
    ...
    if args.configs:
        ...
        if RICH_AVAILABLE:
            ...
            print("=" * 80)
            print(f"Config directory: {generated_dir}")
            print("(Configs will be preserved. Use --cleanup to delete after run)")
            print("=" * 80)
        ...
        if RICH_AVAILABLE:
            ...
            print(f"\n✓ Generated {len(config_files)} test configurations\n")
    ...
    if RICH_AVAILABLE:
        ...
        for i, config_file in enumerate(config_files, 1):
            print(f"[{i}/{len(config_files)}] Verifying {config_file.name}...", end=' ')
            result = run_single_verification(
                config_file,
                modes=args.modes,
                svd=args.svd,
            ...
            results.append(result)
            print(f"{result.status_symbol} ({result.duration:.2f}s)")
        ...
        if args.cleanup:
            ...
            try:
                ...
                if RICH_AVAILABLE:
                    ...
                    print(f"Warning: Could not clean up {generated_dir}: {e}")
        else:
            if RICH_AVAILABLE:
                console = Console()
                console.print(f"\n[cyan]Config files preserved at:[/cyan] [bold]{generated_dir}[/bold]")
            else:
                print(f"\nConfig files preserved at: {generated_dir}")
