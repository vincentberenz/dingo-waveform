...
def plot_polarizations_time(
    ...
    """
    Interactive time-domain plot of h+ and h× strain.
    ...
    title : str
        Plot title
    show_both : bool
        If True, show both h+ and h× on same plot. If False, create subplots.
    height : int
    ...
    if show_both:
        # Single plot with both polarizations
        fig = go.Figure()
    ...
    if plot_type == "amplitude":
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name="∠h<sub>+</sub>",
                line=dict(color="#1f77b4", width=1.5),
                hovertemplate="f=%{x:.2f}Hz<br>∠h<sub>+</sub>=%{y:.2f}rad<extra></extra>",
            )
        )
...
def plot_polarization_spectrogram(
    ...
    polarization : Polarization
        Polarization object
    domain : Domain
        Domain specification (will convert to time domain if needed)
    ...
    # Convert to gwpy for spectrogram calculation
    ...
    if polarization_type == "both":
        # Create side-by-side spectrograms
        fig = make_subplots(
            rows=1,
            cols=2,
            subplot_titles=("h<sub>+</sub>", "h<sub>×</sub>"),
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                z=np.log10(spec_plus.value.T + 1e-30),  # Add small value to avoid log(0)
                colorscale="Viridis",
                colorbar=dict(title="log₁₀(Amplitude)", x=0.45),
                hovertemplate="t=%{x:.3f}s<br>f=%{y:.1f}Hz<br>log₁₀(amp)=%{z:.2f}<extra></extra>",
            ),
            row=1,
            col=1,
        )
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                colorscale="Viridis",
                colorbar=dict(title="log₁₀(Amplitude)", x=1.0),
                hovertemplate="t=%{x:.3f}s<br>f=%{y:.1f}Hz<br>log₁₀(amp)=%{z:.2f}<extra></extra>",
            ),
            row=1,
            col=2,
        ...
        fig.add_trace(
            go.Heatmap(
                ...
                colorscale="Viridis",
                colorbar=dict(title="log₁₀(Amplitude)"),
                hovertemplate="t=%{x:.3f}s<br>f=%{y:.1f}Hz<br>log₁₀(amp)=%{z:.2f}<extra></extra>",
            )
        )
...
def plot_polarization_qtransform(
    ...
    gravitational wave mergers. It provides better time-frequency resolution
    than standard spectrograms.
    ...
    # Convert to gwpy for Q-transform calculation
    h_plus_ts, h_cross_ts = polarization_to_gwpy_timeseries(polarization, domain)
    ...
    if polarization_type == "both":
        # Create side-by-side Q-transforms
        fig = make_subplots(
            rows=1,
            cols=2,
            subplot_titles=("h<sub>+</sub>", "h<sub>×</sub>"),
