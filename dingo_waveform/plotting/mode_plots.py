...
def plot_mode_amplitudes(
    ...
    """
    Bar chart or heatmap of mode amplitudes.
    ...
    """
    # Extract mode labels and amplitudes
    # Note: Mode is an int representing m (azimuthal mode number)
    # For (ℓ,m) harmonics, typically ℓ ≥ |m|
    mode_labels = []
    amplitudes = []

    for m, polarization in sorted(modes.items()):
        # Modes are represented as just m values (int)
        # For standard GW modes, this is typically (2,m) for dominant quadrupole
...
def plot_individual_modes(
    ...
    if title is None:
        pol_name = "h<sub>+</sub>" if polarization_type == "plus" else "h<sub>×</sub>"
...
def plot_mode_comparison(
    ...
    metric : {'amplitude_ratio', 'phase_difference'}
        What to compare: amplitude ratios or phase differences
    polarization_type : {'plus', 'cross'}
        Which polarization to analyze
    title : str, optional
        Plot title (auto-generated if None)
    ...
    for m, polarization in sorted(modes.items()):
        if m == reference_mode:
            continue  # Skip reference mode
        h = polarization.h_plus if polarization_type == "plus" else polarization.h_cross

        if metric == "amplitude_ratio":
            # Compute |h_mode| / |h_ref|
            ratio = np.abs(h) / (np.abs(ref_h) + 1e-30)  # Avoid division by zero
            y_values = ratio
            y_label = f"Amplitude ratio (relative to {reference_mode})"
        else:  # phase_difference
            # Compute angle(h_mode) - angle(h_ref)
...
def plot_modes_grid(
    ...
    """
    Grid of subplots, one per mode. Good for getting an overview.
...
def plot_mode_reconstruction(
    ...
    """
    Reconstruct total h+/h× from selected modes and compare to full sum.

    This is useful for understanding which modes contribute most to the
    total waveform.
    ...
    # Partial reconstruction (selected modes)
    ...
    # Create figure with subplots for h+ and h×
    fig = make_subplots(
        rows=2,
        cols=1,
        subplot_titles=("h<sub>+</sub>", "h<sub>×</sub>"),
        vertical_spacing=0.12,
    )
