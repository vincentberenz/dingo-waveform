...
def check_function_signature(fn: Callable, expected_param_types: List[type], expected_return_type: type) -> bool:
    """
    ...
    # Check return type
    if return_annotation is not inspect.Signature.empty and return_annotation != expected_return_type:
        return False
    ...
    for name, param in params.items():
        if param.default is inspect.Parameter.empty and type_hints.get(name, None) not in expected_param_types:
            return False
